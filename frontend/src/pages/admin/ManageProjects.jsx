...
const ManageProjects = () => {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                const response = await axios.get('http://localhost:8000/api/v1/company/allProjects');
        ...
        try {
            ...
            setProjects((prevProjects) =>
                prevProjects.map((project) =>
                    project._id === projectId ? { ...project, status: project.status === "Active" ? "Inactive" : "Active" } : project
                )
            );
        } catch (error) {
