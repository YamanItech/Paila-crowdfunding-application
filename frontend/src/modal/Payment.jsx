...
function Payment({ open, setOpen, amount, data  }) {
    const selectedPerkId = localStorage.getItem("selectedPerkId");
    const handlePayment = async (paymentGateway) => {
        try {
            ...
            // Make sure selectedPerkId is available
            if (!selectedPerkId) {
                console.error("No perk selected");
                alert("Please select a perk before proceeding with payment");
                return;
            }
            ...
            const payload = {
                ...
                perk: selectedPerkId, // This is the correct usage
                uuid
            };

            console.log("Initiating payment with payload:", payload);

            const response = await axios.post(`${import.meta.env.VITE_BACKEND}/api/initiate-payment`, payload);
